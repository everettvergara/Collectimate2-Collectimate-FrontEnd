...
const props = defineProps({
    ...
    entities: Array,
    statuses: Array,
    prefillEntityId: { type: Number, default: null },
});
const form = useForm({
    entity_id: props.campaign?.entity_id ?? props.prefillEntityId ?? null,
...
const cancelHref = props.campaign
    ? route('campaigns.show', props.campaign.id)
    : props.prefillEntityId
        ? route('entities.show', props.prefillEntityId)
    ...
    <AppLayout>
        ...
        <form class="max-w-xl space-y-4" @submit.prevent="submit">
            ...
            <div>
                ...
                <Input v-model="form.campaign_code" class="w-full" />
                <InputError :message="form.errors.campaign_code" />
            </div>
            <div>
